...
export async function listProducts(filter: ProductFilterRequest = {}): Promise<any> {
...
export async function bulkDeleteProducts(productIds: number[]): Promise<BulkDeleteResponse> {
    const response = await apiClient.post('/pm/products/bulk-delete', { productIds });
...
export async function adjustStock(
    ...
    const response = await apiClient.post(`/pm/products/${productId}/adjust-stock`, request);
    return handleResponse(response);
}
...
export async function rejectRefund(orderId: number, reason?: string): Promise<OrderSummary> {
    const response = await apiClient.post(`/admin/orders/${orderId}/reject`, null, {
        params: { reason },
    });
    return handleResponse<OrderSummary>(response);
}
